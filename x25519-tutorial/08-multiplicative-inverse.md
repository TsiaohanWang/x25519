...
即：先递归算出 $a^i$，平方得到 $a^{2i}$；若指数是奇数再乘一个 $a$ 得到 $a^{2i+1}$。从指数 $e$ 的**最高位**往下扫描：每处理一位，先把当前结果平方；如果该位是 1，再乘一次底数。这样只需要 $O(\log e)$ 次乘法——对一般 255 位指数平均约一半位为 1（约 126 次乘法），最多 255 次；本教程关心的指数 $p - 2$ 几乎全 1（仅 bit 2、bit 4 为 0），见 8.4 的精确计数（254 次平方 + 252 次乘法）。
...
十六进制是 `0x7f` 后跟 30 个 `ff`，再 `eb`（共 64 个十六进制字符）。这是**大端**写法；按第 5 章的小端约定（字节 0 是最低有效位），32 字节数组是 `eb ff ff … ff 7f`——对照 $p = 2^{255}-19$ 的数组 `ed ff … ff 7f`，减 2 后**最低字节**由 `ed` 变 `eb`。
