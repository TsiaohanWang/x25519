...
**元素范围的纪律**。延迟归约能安全进行，前提是每个中间值都留在 64 位槽的安全范围内。下表总结各运算之后元素的典型范围——它是第 7 章溢出分析的前提，论文 §3.1–3.2 与 §4.6 反复使用这组范围：

| 运算 | 输入元素范围 | 输出元素范围 |
|------|--------------|--------------|
| `unpack25519` | 32 字节 | $[0, 2^{16})$（$out[15] \in [0, 2^{15})$，因 bit 255 被清零）|
| `fadd` | 各 $\le 2^{16}$（`fmul` 输出附近）| $[0, 2^{17}]$ |
| `fsub` | 同上 | $[-2^{16}, 2^{16}]$ |
| `fmul`（两次 `carry25519` 后）| 输入必须在 $[-2^{16}, 2^{17}]$ | $[0, 2^{16}]$ 附近（可能略超 `0xffff`）|
| `pack25519`（三次 `carry25519` 后）| 任意 | $[0, 2^{16}-1]$ 严格 |

最关键的是 `fmul` 的输入约束：**元素必须在 $[-2^{16}, 2^{17}]$ 内**，否则第 7.4 节的溢出分析不成立。Montgomery 阶梯（第 12 章）的结构恰好保证这一点：`fmul` 的输出（两次 `carry25519` 后约 $[0, 2^{16}]$）在成为下一次 `fmul` 的输入之前，**至多经历一次 `fadd` 或 `fsub`**，所以输入范围落在 $[-2^{16}, 2^{17}]$。读者现在只需记住这张表，第 7、12 章会回来引用它。
