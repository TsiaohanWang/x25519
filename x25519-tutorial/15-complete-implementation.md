...
对照第 12、14 章的推导，有几个细节值得在这里点破：

- **初始化 `(1:0)` 与 `(x_in:1)`**：`z0 = 0` 正是无穷远点 $\infty$ 的射影表示（第 11.3 节：$Z = 0$ 且 $X \ne 0$），对应 $L(P, 0) = (\infty, P)$；`z1 = 1` 是因为 `x25519` 传入的 `z_in` 是 1（仿射点 $(x:1)$）。`z0` 全零而 `x0[0] = 1` 满足 "$X \ne 0$"的合法性要求。
- **位提取 `(k[i/8] >> (i%8)) & 1`**：`k` 是 32 字节小端数组，第 $i$ 位在字节 `k[i/8]` 的第 `(i%8)` 位；循环 `i = 254 … 0` 共 255 轮（第 13 章钳位保证 bit 255 恒为 0、bit 254 恒为 1）。
- **`x25519` 的五步**：钳位 → 解包（`z = 1`）→ 阶梯 → 一次除法（`finverse` + `fmul`）→ 打包。这五步不是随意拼的：前两步准备输入，第三步在射影坐标里完成全部 255 轮（无除法），第四步把射影结果变回仿射（整个标量乘法唯一一次除法），第五步输出规范字节。

### 性能：一次 X25519 的运算量

把所有 `fmul` 数一遍，可以量化"Curve25519 为什么快"：

| 部分 | `fmul` 次数 |
|------|-------------|
| Montgomery 阶梯：255 轮 × 每轮 10 次 | 2550 |
| `finverse`：254 次平方 + 252 次乘法 | 506 |
| 射影 → 仿射：`fmul(result, x_out, inv_z)` | 1 |
| 合计 | 约 3057 |

每次 `fmul` 内部是 256 次 64 位乘法（16×16 长乘法）加归约与两次 `carry25519`，约 300 次 64 位运算。于是整个 X25519 大约是 $3057 \times 300 \approx 10^6$ 次 64 位运算——在 2010 年后的通用 CPU 上，这就是亚毫秒到几毫秒的量级。相比之下，第 8 章指出 `finverse` 约占阶梯（2550 次）的五分之一（506 次），所以**阶梯的 10 次乘法/轮优化（第 12.5 节）每轮省 4 次、255 轮共砍掉了约 1020 次 `fmul`**——这就是论文 §4.6 那页推导的工程价值。

### 与 RFC 7748 §5 的逐行对照

RFC 7748 §5 用伪代码定义了 `X25519(k, u)`。与我们的实现逐行对应：

```
RFC 7748 §5                          本仓库 x25519.c
────────────────────────────          ─────────────────────────
k = decodeScalar25519(k)              clamp(clamped_sk)（第 13 章）
u = decodeUCoordinate(u, 255)         unpack25519(x, pk)（含 bit 255 掩码）
x1 = u                                （x_in 即 u）
(x2, z2) = (1, 0)                     x0 = 1, z0 = 0
(x3, z3) = (u, 1)                     x1 = x_in, z1 = 1
for t = 254 down to 0                 for (i = 254; i >= 0; --i)
    k_t = (k >> t) & 1                bit = (k[i/8] >> (i%8)) & 1
    swap ^= k_t                       （无——见下文"交换技巧"）
    cswap(x2, x3, swap)                swap25519(x0, x1, bit)
    cswap(z2, z3, swap)                swap25519(z0, z1, bit)
    A = x2 + z2; AA = A^2             fadd/fmul（倍增分支）
    B = x2 - z2; BB = B^2             fsub/fmul
    E = AA - BB                       fsub
    C = x3 + z3; D = x3 - z3          fadd/fsub（差分加法分支）
    DA = D * A; CB = C * B            fmul ×2
    x3 = (DA + CB)^2                  fmul(x1, t7, t7)
    z3 = x1 * (DA - CB)^2             fmul(z1, x_in, t8)
    x2 = AA * BB                      fmul(x0, t1, t2)
    z2 = E * (AA + a24 * E)           fmul(z0, t3, t5)（本仓库 a24_fe = 121666，配 $(x_0-z_0)^2$ 那一路；RFC 的 a24 = 121665 配 $AA$，见下方“差异 2”）
x2 = x2 * z2^(p-2)                    finverse + fmul
return encodeUCoordinate(x2, 255)     pack25519(out, result)
```

两处值得注意的差异：

1. **交换技巧**：RFC 用一个 `swap` 变量累加（`swap ^= k_t`），每轮对 $x$、$z$ 各 `cswap` **一次**（共两次调用），循环结束后再补一次最终 `cswap`；我们每轮做**四次** `swap25519`（阶梯前换过去、阶梯后换回来）。两者数学上完全等价：RFC 的写法让“处理完的位”保持在原位，我们的写法是“先换过去算、再换回来”。都是常数时间（`cswap`/`swap25519` 无分支）。RFC 的写法少一半交换开销，但需要 `swap` 寄存器；我们的写法更接近公式 (28) 的原始表述、易于对照教学。
2. **常数组合**：RFC 用 `a24 = 121665 = (A-2)/4` 配 `AA + a24·E`（即 $(x+z)^2$ 那一路），我们用 `121666 = (A+2)/4` 配 $(x-z)^2$ 那一路。第 12.6 节证明两者结果相同。**注意 RFC 的 `a24` 与我们的 `a24_fe` 名字相同、数值不同**（121665 vs 121666），阅读其他实现时务必看清它配的是哪一项。
...
6. 对照 15.3 节的 RFC 对照表：RFC 的 `z2 = E * (AA + a24*E)` 对应 `montgomery_ladder` 中哪几行？若把 `a24_fe` 换成 RFC 的 `121665` 并改用 $(x0+z0)^2$ 那一路（RFC 的 `AA`），倍增分支需要调整哪几行？
