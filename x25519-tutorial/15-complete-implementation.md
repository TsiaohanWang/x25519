...
每次 `fmul` 内部是 256 次 64 位乘法（16×16 长乘法）加归约与两次 `carry25519`，约 300 次 64 位运算。于是整个 X25519 大约是 $3057 \times 300 \approx 10^6$ 次 64 位运算——在 2010 年后的通用 CPU 上，这就是亚毫秒到几毫秒的量级。相比之下，第 8 章指出 `finverse` 本身约占五分之一（506 次），所以**阶梯的 10 次乘法/轮优化（第 12.5 节）直接砍掉了约 1275 次 `fmul`**——这就是论文 §4.6 那页推导的工程价值。
...
for t = 254 down to 0                 for (i = 254; i >= 0; --i)
    ...
    z2 = E * (AA + a24 * E)           fmul(z0, t3, t5)（a24 = 121666）
