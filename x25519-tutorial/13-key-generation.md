...
恒为单位元，攻击者得不到私钥信息。对合法的阶 $q$ 基点，$8k'$ 与 $k'$ 产生的分布相同（$8$ 与 $q$ 互素），所以无害。
...
另外，钳位还有一个**正确性**理由：如果 $k = 0$ 或 $k$ 是 $q$ 的倍数，则$P^k = \infty$。`scalarmult` 只能返回 $x$ 坐标，无法区分“$x$ 坐标为 0 的点”和“$\infty$”（两者都输出 0）。强制 $0 < k < q$（结合 (1)(2)(3) 得 $k \in [8 \cdot 2^{251}, 2^{255}-1]$，且 $k < 8q$ 基本成立）避免了正常协议中产生 $\infty$ 的情况。
...
> **勘误**：旧版教程引用 `a0a1…bf → e6db…` 并称其为 “RFC 7748 测试向量”，这是**错误的**——RFC 7748 中不存在这组数据（`e6db…` 是 §5.2 向量 1 的 u 坐标输入，
