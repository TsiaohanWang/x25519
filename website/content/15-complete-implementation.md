...
1. **交换技巧**：RFC 用一个 `swap` 变量累加（`swap ^= k_t`），每轮只 `cswap` **一次**；我们每轮交换**两次**（阶梯前一次、阶梯后一次交换回来）。两者数学上完全等价：RFC 的写法让"处理完的位"保持在原位，我们的写法是"先换过去算、再换回来"。都是常数时间（`cswap`/`swap25519` 无分支）。RFC 的写法少一半交换开销，但需要 `swap` 寄存器；我们的写法更接近公式 (28) 的原始表述、易于对照教学。
