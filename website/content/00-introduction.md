...
2. **修正了测试向量**。旧版引用的 "RFC 7748 测试向量"（`a0a1…bf → e6db…` 等）并非
   RFC 7748 的真实数据，导致验证程序失败。本版全部改用 RFC 7748 §5.2 与 §6.1 的
   真实测试向量（详见第 13、14 章与 `test.c`）。
3. **修正了代码错误**。旧版第 12/15/16 章中 `montgomery_ladder` 的位索引错误
   （`i/64` 应为 `i/8`）、第 11 章点倍增公式错误、`(A±2)/4` 常数混淆等，本版均已
   修正，并保证教程中逐步构建的代码与最终的 `x25519.c` 完全一致。
4. **消除重复**。旧版第 4 章与第 10 章、第 5 章与第 9 章内容大量重复，本版重新划分：
   第 4 章专注椭圆曲线群的几何构造，第 10 章专注 Curve25519 的具体参数与安全性。
5. **强化"为什么"**。每个看似神奇的数字（38、121665/121666、三次 `carry25519`、
   钳位的三个比特、阶梯中的因子 4……）都解释了来源。
...
教程正文与代码基于 Kleppmann 论文与 TweetNaCl，均为可自由使用的内容；
本仓库代码遵循公共领域（public domain）许可。
