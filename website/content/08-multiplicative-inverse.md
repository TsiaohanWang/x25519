...
域算术的最后一个运算是**除法**。第 3 章说过：$\frac{b}{a} = b \cdot a^{-1}$，而
$a^{-1} \equiv a^{p-2} \pmod{p}$（费马小定理）。本章回答：
...
要算 $a^{e}$（$e = p - 2$ 是 255 位整数），朴素做法要 $e$ 次乘法——不可行。平方-乘法
利用指数二进制展开的递归关系：

$$a^{2i} = a^i \cdot a^i, \qquad a^{2i+1} = a \cdot a^i \cdot a^i$$

即：先递归算出 $a^i$，平方得到 $a^{2i}$；若指数是奇数再乘一个 $a$ 得到 $a^{2i+1}$。
从指数 $e$ 的**最高位**往下扫描：每处理一位，先把当前结果平方；如果该位是 1，
再乘一次底数。这样只需要 $O(\log e)$ 次乘法——对 255 位指数大约 255 次平方和
约一半的乘法。
...
所以低 8 位是 `11101011`：**bit 0、1、3、5、6、7 为 1，bit 2 和 bit 4 为 0**；
从 bit 8 到 bit 254 全部为 1。总结：
...
十六进制是 `0x7f` 后跟 30 个 `ff`，再 `eb`（共 64 个十六进制字符，即 32 字节的
小端编码 `7f ff ff … ff eb`；对照 $p = 2^{255}-19$ 的编码 `…ed`，减 2 后末字节变 `eb`）。
...
1. **`c` 初始化为 `in` 而不是 1**。标准平方-乘法从 `c = 1` 开始、从 bit 254 扫到
   bit 0：第一轮（i=254）`c = 1² = 1`，然后因为 bit 254 是 1，`c = 1 · in = in`。
   论文直接让 `c = in`，等价于**已经完成**了 bit 254 的处理，于是循环从
   **bit 253** 开始——少做一次迭代。这就是"从 253 开始"的来历，不是笔误。
2. **`if (i != 2 && i != 4)`**。$p - 2$ 中只有 bit 2 和 bit 4 是 0，其余位是 1；
   平方-乘法只在该位为 1 时乘 `in`，所以这里**跳过**这两个特定位。这个条件
   只依赖公开的循环变量 $i$，不是秘密——因此它是常数时间的安全分支。
3. 循环结束后 `c = in^{p-2} \bmod p$ = in 的乘法逆元。整个序列是固定的 254 次
   平方 + 252 次乘法，执行时间与 `in` 的值无关。

**耗时**：254 次平方 + 252 次乘法 ≈ 506 次 `fmul`。对比第 12 章 Montgomery 阶梯
（255 次迭代 × 10 次乘法 ≈ 2550 次），逆元约占五分之一——这就是为什么
...
所以是常数时间。对比：扩展欧几里得算法（第 3 章提过）的迭代次数随输入变化，
不是常数时间，因此被排除。
...
> 的结果在模 $p$ 意义下是 1，未完全归约的表示中低位 limb 已经是 1。
> （严格验证请参考 `test.c` 里基于 RFC 向量的端到端测试。）
...
4. 如果 `if (i != 2 && i != 4)` 改成 `if (in[0] & 1)` 会怎样？为什么那会破坏
   常数时间？
...
下一章完成域算术的最后一块：把 `field_elem` 打包回字节数组（`pack25519`），
顺便实现常数时间条件交换 `swap25519`。
